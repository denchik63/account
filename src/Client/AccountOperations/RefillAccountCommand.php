...
#[AsCommand(name: 'app:refill-account')]
class RefillAccountCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln(sprintf('Refilling account with id "%s" on sum "%f"...', $accountId, $sum));

        $requestUid = Uuid::v7();
        $request = $this->createRequestData($requestUid, $accountId, $sum);

        $output->writeln(sprintf('Sending refilling request with uid "%s"', $requestUid));
            ...
            $this->messaging->handleResponse($queueName, function (BaseMessageInterface $message) use ($output, $requestUid): void {
                ...
                if ($this->validateResponseData($responseAsArray, $requestUid)) {
                    if (empty($responseAsArray['success'])) {
                        ...
                        $output->writeln('<success>Account successfully refilled</success>');
                    }
    ...
    {
        return json_encode([
            'name' => EventNames::AccountRefill->value,
            'uid' => $requestUid,
